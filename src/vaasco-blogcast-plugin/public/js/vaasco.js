(function () {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        var script = document.createElement('script'),
            pollySupported = pollyIsSupported(),
        ...
        if (!webComponentsSupported && pollySupported) {
            script.async = true;
        ...
        function lazyLoadPolymerAndElements() {
            var link = document.createElement('link'),
                widget,
                options = document.getElementById("vaasco").getAttribute("data-options"); 

            link.setAttribute('rel', 'import');
            link.setAttribute('href', '/wp-content/plugins/vaasco-blogcast/public/components/vaasco-widget.html');
            
            document.head.appendChild(link);

            if (!document.getElementsByTagName('vaasco-widget')[0]) {
                widget = document.createElement('vaasco-widget');
                widget.setAttribute('options', options);
                document.body.insertBefore(widget, document.body.childNodes[0]);
            }
        }

        function pollyIsSupported() {
            if ((bowser.chrome && bowser.version >= 40) ||
                (bowser.firefox && bowser.version >= 35) ||
                (bowser.opera && bowser.version >= 35) ||
                (bowser.msie && bowser.version >= 11) ||
                (bowser.msedge) ||
                (bowser.safari && bowser.version >= 9) ||
                (bowser.ios && bowser.version >= 9)) {
                return true;
            }
